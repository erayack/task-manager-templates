...
import { Task } from "../../configs/types.template";
import { StatusBadge, getStatusBadgeData } from "../atoms/StatusBadge.template";
import { PriorityIndicator, getPriorityBadgeData } from "../atoms/PriorityIndicator.template";

// TEMPLATE: Simple formatting functions - replace with your formatters
const formatTaskTitle = (title: string, maxLength: number) => 
  title.length > maxLength ? `${title.substring(0, maxLength)}...` : title;

const formatTaskId = (id: string) => `ID: ${id}`;

const generateTaskSubtitle = (task: Task, options: { includeComplexity?: boolean; includeMetadata?: boolean }) => {
  const parts = [formatTaskId(task.id)];
  if (options.includeComplexity && task.complexityScore) {
    parts.push(`Complexity: ${task.complexityScore}/10`);
  }
  if (options.includeMetadata && task.dependencies?.length) {
